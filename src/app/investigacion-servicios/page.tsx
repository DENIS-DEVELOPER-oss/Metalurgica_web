...
const quickNavItems = [
  { title: 'Líneas de Investigación', icon: <Microscope className="h-8 w-8 mb-2" />, href: '#lineas-investigacion', color: 'bg-blue-500 hover:bg-blue-600' },
  { title: 'Ambientes Académicos', icon: <School className="h-8 w-8 mb-2" />, href: '#ambientes-academicos', color: 'bg-green-500 hover:bg-green-600' },
  { title: 'Investigadores', icon: <Users className="h-8 w-8 mb-2" />, href: '#investigadores', color: 'bg-yellow-500 hover:bg-yellow-600 text-card-foreground' },
  { title: 'Publicaciones', icon: <BookOpen className="h-8 w-8 mb-2" />, href: '#publicaciones', color: 'bg-red-500 hover:bg-red-600' },
  { title: 'Repositorio de Tesis', icon: <GraduationCap className="h-8 w-8 mb-2" />, href: 'http://repositorio.unap.edu.pe/', external: true, color: 'bg-purple-500 hover:bg-purple-600' },
  { title: 'Laboratorios y Gabinetes', icon: <FlaskConical className="h-8 w-8 mb-2" />, href: '#laboratorios-gabinetes', color: 'bg-indigo-500 hover:bg-indigo-600' },
  { title: 'Campus Virtual', icon: <Laptop className="h-8 w-8 mb-2" />, href: 'https://campus.unap.edu.pe/', external: true, color: 'bg-pink-500 hover:bg-pink-600' },
];
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      ...
      <section className="mb-16">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-4">
          {quickNavItems.map((item) => (
            <Button key={item.title} asChild className={`flex flex-col items-center justify-center p-4 rounded-lg text-white text-center h-32 shadow-md transition-colors duration-300 ${item.color}`}>
      ...
      <section id="lineas-investigacion" className="mb-16 scroll-mt-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {researchLines.map((line) => (
            <Card key={line.title} className="shadow-lg">
              <CardHeader>
      ...
      <section id="ambientes-academicos" className="mb-16 scroll-mt-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {academicEnvironments.map((env) => (
            <Card key={env.title} className="shadow-lg overflow-hidden">
              <Image src={env.imageUrl} alt={env.title} width={600} height={400} className="w-full h-48 object-cover" data-ai-hint={env.imageHint} />
      ...
      <section id="investigadores" className="mb-16 scroll-mt-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {researchers.map((researcher) => (
            <Card key={researcher.name} className="shadow-lg">
              <CardContent className="p-6">
                <div className="flex flex-col sm:flex-row items-center mb-4">
                  <Image src={researcher.imageUrl} alt={researcher.name} width={96} height={96} className="w-24 h-24 rounded-full object-cover mr-0 sm:mr-6 mb-4 sm:mb-0 flex-shrink-0" data-ai-hint={researcher.imageHint} />
                  <div>
                ...
                <Button asChild className={`${researcher.buttonColor} text-primary-foreground`}>
        ...
        <Card className="shadow-lg">
          <CardContent className="p-6">
            <h3 className="text-xl font-semibold mb-4 text-primary">
              <Link href="https://dialnet.unirioja.es/servlet/articulo?codigo=5319723" target="_blank" rel="noopener noreferrer" className="hover:underline">
      ...
      <section id="laboratorios-gabinetes" className="mb-12 scroll-mt-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {labs.map((lab) => (
            <Card key={lab.title} className="shadow-lg overflow-hidden">
              <Image src={lab.imageUrl} alt={lab.title} width={600} height={400} className="w-full h-48 object-cover" data-ai-hint={lab.imageHint} />
              <CardContent className="p-6">
